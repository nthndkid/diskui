...
public class WordGuess {
	...
	//code incorrect guess message box
	@SuppressWarnings("static-access")
	private void validateGuess(char letter) {
	    boolean correct = false;
	    char[] charWord = displayedWord.toCharArray();
	    
	    boolean anyMatch = false; // Track if any match is found

	    for (int ctr = 0; ctr < pickedWord.length(); ctr++) {
	        if (givenLetterIndex1 == ctr || givenLetterIndex2 == ctr) {
	            continue; // Skip over the given letters positions
	        } else {
	    ...
	    if (!anyMatch) {
	        // If no match was found in the entire word
	        JOptionPane.showMessageDialog(frame, "Incorrect letter! Try again.");
	        mistakes++;
	    } else {
	        displayedWord = new String(charWord); // Update displayed word
